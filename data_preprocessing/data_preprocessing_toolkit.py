...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_long_stays(df):
        ...
        return df.query("length_of_stay < 22")
    ...
    def filter_out_low_prices(df):
        ...
        return df.query("accommodation_price > 50")
    ...
    def add_weekend_stay(df):
        ...
        # Write your code here #
        ########################
        def is_weekend_stay(row):
            for day in range(row["length_of_stay"]):
                if 6 > (row["date_from"] + timedelta(day)).weekday() > 4:
                    return "True"
            return "False"

        df['weekend_stay'] = df.apply(is_weekend_stay, axis=1)
        return df
    ...
    def add_night_price(df):
        ...
        df["night_price"] = df["night_price"].apply(lambda x: float("{0:.2f}". format(x)))
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        groups = [
            group_reservations.loc[:, ["group_id"] + self.sum_columns].groupby("group_id").sum(),
        ...
        group_reservations = groups[0]
        for i in range(1, len(groups)):
            group_reservations = group_reservations.merge(groups[i], on="group_id")
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        # Write your code here #
        ########################
        averages = df.query("accommodation_price > 0").groupby("room_group_id").mean()["night_price"].reset_index()
        df = df.drop(columns="night_price")
        df = pd.merge(df, averages, on="room_group_id", how="left")
